...
var _Today = require("@material-ui/icons/Today");
...
var DateFormatInput = /** @class */function (_super) {
    ...
    DateFormatInput.prototype.render = function () {
        ...
        var _b = this.state,
            focus = _b.focus,
            calendarShow = _b.calendarShow;
        return [React.createElement("div", { key: 'date-input', className: className, ref: function ref(input) {
                return _this.input = input;
            } }, React.createElement(_FormControl2.default, { className: classes.formControl, disabled: disabled, onClick: this.toggleShowCalendar, error: error !== undefined, fullWidth: true }, label && React.createElement(_InputLabel2.default, __assign({ shrink: focus || calendarShow || value !== undefined, htmlFor: name }, __assign({}, InputLabelProps, { classes: InputLabelProps && InputLabelProps.classes ? __assign({ root: classes.label }, InputLabelProps.classes) : { root: classes.label } })), label), React.createElement(_Input2.default, __assign({ name: name, value: value ? this.dateValue(value) : "\xA0", onFocus: function onFocus() {
                ...
                } }, endIcon ? endIcon : React.createElement(_Today2.default, null))) }, InputProps)), error && React.createElement(_FormHelperText2.default, __assign({ error: true }, FormHelperTextProps), error))), dialog ? React.createElement(_Dialog2.default, { key: 'date-dialog', open: calendarShow, onClose: this.closeCalendar }, React.createElement(_calendar2.default, __assign({ ref: function ref(calendar) {
                return _this.calendar = ReactDOM.findDOMNode(calendar);
            }, value: value, onChange: onChange, min: min, max: max, closeCalendar: this.closeCalendar, okToConfirm: okToConfirm }, CalendarProps))) : React.createElement(_Popover2.default, { key: 'date-popover', onEntered: function onEntered() {
                if (_this.action.resize) _this.action.resize();
            }, open: calendarShow, anchorOrigin: anchorOrigin, transformOrigin: transformOrigin, anchorEl: this.input }, React.createElement(_calendar2.default, __assign({ action: function action(_action) {
                return _this.action.resize = _action.resize;
            }, ref: function ref(calendar) {
                return _this.calendar = ReactDOM.findDOMNode(calendar);
            }, value: value, onChange: onChange, min: min, max: max, closeCalendar: this.closeCalendar, okToConfirm: okToConfirm }, CalendarProps)))];
    };
    DateFormatInput = __decorate([(0, _styles.withStyles)(styles)], DateFormatInput);
